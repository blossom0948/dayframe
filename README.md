...
같은 저장소의 루트에는 APK 화면을 그대로 검토할 수 있는 모바일 폭의 브라우저용 Dayframe 뷰어도 포함되어 있습니다. 별도 PC용 레이아웃은 만들지 않으며, [CLOUDFLARE_PAGES.md](CLOUDFLARE_PAGES.md)의 Pages 설정으로 Android 프로젝트와 모바일 웹 미리보기를 함께 관리할 수 있습니다.
...
웹에서는 APK와 같은 달력·피드·통계·보관함 하단 탭을 검토하고, 사진 파일을 선택해 새 기록을 만들거나 편집할 수 있습니다. 기록은 브라우저에만 저장되며 보관함에서 JSON으로 백업/복원할 수 있습니다.

```powershell
npm.cmd run build
```

빌드 결과는 `dist/`이며 Cloudflare Pages의 출력 폴더로 사용합니다.
